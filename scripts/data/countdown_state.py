...
if __name__ == "__main__":
    ...
    parser.add_argument("--local_dir", default="countDown-state")
    ...
    data_source = "Parallel-Reasoning/countdown_problems"
    ...
    def make_map_fn(split):
        def process_fn(example, idx):
            ...
            nums = example.pop('nums')
            sorted_nums = sorted(nums)

            prompt = f"""Using the numbers {nums}, create an equation that equals {target}.
You can use the basic arithmetic operations (+, -, *, /).
Each number can only be used once.
All divisions must be exact.

State Representation:
After each reasoning step, you must explicitly write the current state.

The state represents the multiset of remaining numbers and the target.

The state is written using the following format:

<state>
numbers: [list of remaining integers]
target: {target}
</state>

The numbers must be listed in ascending order.
The state must list all remaining numbers.
Any number not listed is assumed to have been used already.
...
Show your work using the following repeated format:

<think>
Explain your reasoning about which numbers to combine next.
</think>
<state>
numbers: [...]
target: {target}
...
You may repeat the above <think> </think> <state> </state> pattern as many times as needed.

After completing your reasoning, return the final equation **only** inside <answer> </answer> tags, for example:
...
"""
            ...
            reward_model = {"style": "rule", "ground_truth": {'numbers': nums, 'target': target}}

            data = {
                "data_source": 'countdown-state',
                "prompt": prompt,
                "ability": "math",
